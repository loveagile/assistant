  ...
  for (i = 0; i < acc.length; i++) {
    acc[i].addEventListener("click", function () {
      ...
      if (panel.style.maxHeight) {
        ...
        document.querySelector('.' + arrow[1].className).style.transform = 'rotateZ(0)';
      } else {
        panel.style.maxHeight = panel.scrollHeight + "px";
        document.querySelector('.' + arrow[1].className).style.transform = 'rotateZ(225deg)';
      }
    });
  }
...
$(window).on("load", function() {
  $(".loading").stop().animate({ opacity: 0}, { "duration": 1800, complete: function() {
    $(this).remove();
  }});
});
